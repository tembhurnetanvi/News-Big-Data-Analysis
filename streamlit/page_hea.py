...
def page_second():
    st.header("HEALTH")
    DATA_URL="https://storage.googleapis.com/storm_event/CNN/CNN_Health.csv"
    ...
    # audio_backend = f'https://news-analysis-es3uwbxn2a-uc.a.run.app/health/{int_val}/text-to-speech'
    # audio = process_tts(audio_backend)
    # if audio:
    #     st.audio(f'https://storage.googleapis.com/storm_event/CNN_audio/health/{int_val}.mp3', format='audio/ogg')
...
def page_third():
    ...
    if nlp_option=="Sentiment":
        st.write("# Sentiment")
        backend = f'https://news-analysis-es3uwbxn2a-uc.a.run.app/health/{x}/sentiment'
    ...
    if nlp_option=="Summarization":
        ...
        backend = f'https://news-analysis-es3uwbxn2a-uc.a.run.app/health/{x}/summarizer'
