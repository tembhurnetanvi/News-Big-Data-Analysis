...
def page_second():
    st.header("OPINION")
    DATA_URL="https://storage.googleapis.com/storm_event/CNN/CNN_Opinion.csv"
    ...
    # audio_backend = f'https://news-analysis-es3uwbxn2a-uc.a.run.app/opinion/{int_val}/text-to-speech'
    # audio = process_tts(audio_backend)
    # if audio:
    #     st.audio(f'https://storage.googleapis.com/storm_event/CNN_audio/opinion/{int_val}.mp3', format='audio/ogg')
...
def page_third():
    ...
    if nlp_option=="Sentiment":
        st.write("# Sentiment")
        backend = f'https://news-analysis-es3uwbxn2a-uc.a.run.app/opinion/{x}/sentiment'
    ...
    if nlp_option=="Summarization":
        ...
        backend = f'https://news-analysis-es3uwbxn2a-uc.a.run.app/opinion/{x}/summarizer'
